...
public class BTree implements BTreeInterface {
				...
				if (pointer.getValue(i) == null) {
					...
					// check node: overload
						...
						// case: leaf
							...
							// neighbour leaves have free storage: re-arrange
							// values
							if (neighbourLeaf != -1) {
	...
	// }
	// else {
	// println("Error: Input file \"" + filename + "\" is either unreadable or
	// does not exist.");
	// return false;
	// }
	// }
	...
	public boolean contains(Object o) {
		...
		do {
			...
			if (parent.getChild(0) == null) // If we reached a leaf, element is
											// not in tree.
				end = true;
	...
	/*
	 * There is no defined convention for pre-, in- or postorder transversion
	 * strategy for working with a B-Tree. We define them as following:
	 * 
	 * (1) Preorder ------------ While using the preorder transversion strategy,
	 * we first enter and look up all of the values contained within the node
	 * and then attempt to repeat the process for each child of the given node.
	 * 
	 * (2) Inorder ------------ While using the inorder transversion strategy,
	 * we first enter and look up all of the values contained within the
	 * left-most node and then enter the smallest value in the node above
	 * (parent). The process is then repeated for each child of the parent node,
	 * while we work our way from the bottom to the top.
	 * 
	 * (3) Postorder ------------- While using the postorder transversion
	 * strategy, we first attempt to look up and enter all of the children nodes
	 * referenced within the given node before entering the given node itself.
	 */
			...
			for (int i = neighbourLeaf; i < child; i++) {
				...
				for (int j = 0; j < pointer.getValues().length - 1 && !set; j++) {
					if (parent.getChild(i).getValue(j) == null) {
						...
						// search and set position for parent value (from the
						// top to the left)
						parent.getChild(i).setValue(parentValue, j);
						set = true;
					}
				}
				// place child value inside parent node (from the left to the
				// top)
	...
	public BTreeNode locateParentNode(Object query) {
			...
			if (result == -1) {
				...
				for (int j = 0; j < validRange; j++) {
					if (((Comparable) query).compareTo(newParent.getChild(i).getValue(j)) == 0) {
						newParent.getChild(i).setValue(null, j); // Remove
																	// located
																	// value
						found = true;
					}
				...
				for (int j = 0; j < validRange; j++) {
					if (((Comparable) query).compareTo(newParent.getChild(i + 1).getValue(j)) == 0) {
						newParent.getChild(i + 1).setValue(null, j); // Remove
																		// located
																		// value
						found = true;
					}
		...
		} while (i < magnitude * 2 && !found); // Value we seek can never be in
												// a temporary node.
	...
	private void treatNode(BTreeNode parent) {
		...
		// See if the child is a leaf-parent so it can try to take largest value
		// from smaller child
	...
	private void treatLeaf(BTreeNode parent) {
		if (!rebalanceLeafs(parent)) {
			...
			if (childCount(parent) > magnitude + 1) {
				...
				// Last child has no values, remove it and push largest parent
				// value into previous leaf
				if (lChild[0] == null) {
					...
					parent.getChild(elementPos).setValue(lastElement,
							freeSpot(parent.getChild(elementPos).getValues()));
					erase(parent, lastElement);
					parent.setChild(null, largeChild);
					return;
				}

				// Copy the corresponding parent's value into smaller child and
				// remove it
				sChild[freeSpot(sChild)] = parent.getValue(smallChild);
				erase(parent, parent.getValue(smallChild));

				// Copy all the values from the larger child into the smaller
				// one
			...
			// Leaf is the largest in its branch
				...
				if (((Comparable) parent.getValue(0)).compareTo(lastElement(root.getValues())) == -1) {
					...
					// Take the root's corresponding value and add it to the end
					// of the leaf
	...
	/**
	 * Performs a branch-spanning right-rotation to move an abundant value
	 * within a leaf into the tree's last leaf
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private void rotateToLastLeaf(Object aVal) {
		...
		insertionSort(eNode.getValues());
		erase(child, aVal);

		// aVal is now the one we pushed aside, move it into the smallest leaf
		// in its left branch
	...
	/**
	 * Finds and erases a value within a node's direct child [do not use if
	 * child is a leaf]
	 */
	...
	private BTreeNode getChild(BTreeNode parent, Object o) {
		...
		while (!found) {
			...
			// If the value is not in any of the sub-arrays then the child is
			// the root.
	...
	/**
	 * Returns the child's array position in a node which contains a specific
	 * value
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private int getChildPosition(BTreeNode parent, Object aVal) {
		...
		while (i <= magnitude * 2) {
			for (int j = 0; j < magnitude * 2; j++) {
				if (parent.getChild(i).getValue(j) != null
						&& ((Comparable) parent.getChild(i).getValue(j)).compareTo(aVal) == 0) {
					return i;
				}
			}
			i++;
	...
	public void rotateRight(BTreeNode parent, int from, int to) {
		...
		for (int i = from; i < to; i++) {
			...
			parent.getChild(i + 1).setValue(pValue, freeSpot(parent.getChild(i + 1).getValues()));
			insertionSort(parent.getChild(i + 1).getValues());
	...
	/**
	 * Searches all leafs for an abundant value and returns it. Returns -1 if
	 * none found
	 */
